...
本实验所使用的数据来自CSMAR(国泰安) http://cn.gtadata.com/，主要包括：
...
对于借壳上市的公司，我们通过添加后缀来创建新的标识符：
- "0"表示非借壳上市公司
- "1"表示借壳上市前的公司
- "2"表示借壳上市后的公司

这种处理方式确保了我们能够正确捕捉借壳上市前后公司特性的变化。
...
这一步我们处理财务数据，包括与公司信息的合并、处理借壳上市数据、填充缺失值等。由于财务数据的披露通常有滞后性，我们需要确保使用最新可获得的财务数据。我们还需要处理一些特殊情况，例如：

- 过滤借壳上市当年的财务数据，避免借壳上市带来的财务信息不连续性
