...
def backtest_ch3_model(stock_returns, market_cap, earnings_to_price, market_return,
    ...
    for date in dates:
        ...
        # 构建规模因子
        smc_results = construct_smc_factor(data)
        ...
        results['SMC'].append(smc_results['SMC'])
        results['VMG'].append(vmg_results['VMG'])
        results['Medium'].append(smc_results['Medium'])
        results['Big'].append(smc_results['Big'])
...
def plot_results(results_df):
    ...
    plt.plot(cumulative_returns.index, cumulative_returns['SMC'], label='规模因子(SMC)')
    ...
    results_df[['MKT', 'SMC', 'VMG']].boxplot()
...
def main():
    ...
    smc_performance = calculate_performance(results['SMC'])
    ...
    print("\n规模因子(SMC)绩效指标:")
    for key, value in smc_performance.items():
        print(f"{key}: {value:.4f}")
